...
class DensityAnimationGenerator(QThread):
    ...
    def generate_frames_from_tracks(self):
        ...
        for i, t in enumerate(time_points):
            ...
            frame_data = self.create_frame_at_time(t)
    ...
    def create_frame_at_time(self, time_point):
        """Create frame data for a specific time point"""
        width, height = self.chamber_dimensions
        
        # Collect all cell positions at this time point
        current_positions = []
        current_velocities = []
        active_tracks = []
        ...
        if current_positions:
            ...
            density_grid = np.zeros((len(grid_y_bins)-1, len(grid_x_bins)-1))
        
        # Calculate frame statistics
        stats = {
            'total_cells': len(current_positions),
            'avg_velocity': np.mean(current_velocities) if current_velocities else 0,
            'max_velocity': np.max(current_velocities) if current_velocities else 0,
            'time_point': time_point,
            'high_density_regions': np.sum(density_grid > 10),
...
class BacterialDensityAnimationModal(QDialog):
    ...
    def __init__(self, motility_dialog, parent=None):
        ...
        self.setWindowTitle("Bacterial Density Evolution Animation")
        self.setModal(True)
    ...
    def create_header(self):
        ...
        # Title
        title_label = QLabel("Bacterial Density Evolution")
        ...
        filtered_count = len(getattr(self.motility_dialog, 'tracked_cells', []))
        all_count = len(getattr(self.motility_dialog, 'lineage_tracks', []))
        
        self.filtered_radio = QRadioButton(f"Filtered Tracks ({filtered_count})")
    ...
    def create_controls_panel(self):
        ...
        counter_label = QLabel("LIVE CELL COUNT")
    ...
    def start_animation_generation(self):
        ...
        if not tracks:
            QMessageBox.warning(self, "No Data", f"No {source_name} tracks available")
            return
    ...
    def generation_finished(self):
        ...
        if self.animation_frames:
            ...
            self.time_label.setText(f"00:00 / {int(total_seconds//60):02d}:{int(total_seconds%60):02d}")
            
        QMessageBox.information(
            self, "Animation Ready", 
            f"Generated {len(self.animation_frames)} frames from bacterial tracking data!"
        )
    ...
    def display_frame(self, frame_idx):
        ...
        if self.show_density_cb.isChecked():
            density_grid = frame_data['density_grid']
        ...
        if self.show_cells_cb.isChecked() and frame_data['positions']:
            x_coords, y_coords = zip(*frame_data['positions'])
            ax.scatter(x_coords, y_coords, s=12, color='cyan', alpha=0.8, edgecolors='blue', linewidth=0.5)
        ...
        # 4. Display cell count prominently on plot
        current_cell_count = frame_data['stats']['total_cells']
        ax.text(0.02, 0.98, f'Cells: {current_cell_count}', 
        ...
        title = f"Bacterial Density Evolution - Time Point {stats['time_point']}"
        ax.set_title(title, fontsize=14, fontweight='bold')
        
        self.canvas.draw()
        self.update_frame_stats(frame_data['stats'])
        
    def update_frame_stats(self, stats):
        """Update statistics display"""
        # Update big cell counter
        self.cell_counter.setText(str(stats['total_cells']))
        
        # Update detailed stats
        stats_text = f"""Frame: {self.current_frame + 1}/{len(self.animation_frames)}
Time Point: {stats['time_point']}

Active Bacteria: {stats['total_cells']}
...
Active Tracks: {stats['active_tracks']}"""
    ...
    def generation_error(self, error_msg):
        ...
        self.progress_bar.setVisible(False)
        self.generate_btn.setEnabled(True)
        QMessageBox.critical(self, "Generation Error", f"Failed to generate animation: {error_msg}")
    ...
    def export_animation(self):
        ...
        default_name = f"bacterial_density_evolution_{source_type}_{timestamp}"
        ...
        try:
            ...
            for i, frame_data in enumerate(self.animation_frames):
                ...
                if self.show_cells_cb.isChecked() and frame_data['positions']:
                    x_coords, y_coords = zip(*frame_data['positions'])
                    ax.scatter(x_coords, y_coords, s=8, color='cyan', alpha=0.8)
                
                # Add cell count
                cell_count = frame_data['stats']['total_cells']
                ax.text(0.02, 0.98, f'Cells: {cell_count}', transform=ax.transAxes, 
                       fontsize=16, fontweight='bold', color='white',
                       bbox=dict(boxstyle='round', facecolor='black', alpha=0.8))
                
                ax.set_xlim(0, width)
                ax.set_ylim(0, height)
                ax.set_title(f"Bacterial Density - Time Point {frame_data['stats']['time_point']}")
...
def add_animation_button_to_motility_widget(motility_dialog):
    ...
    if hasattr(motility_dialog, 'density_layout'):
        animation_btn = QPushButton("🎬 Animate Bacterial Density Evolution")
