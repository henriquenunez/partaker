...
class MotilityDialog(QDialog):
    ...
    def __init__(self, tracked_cells, lineage_tracks, image_data=None, parent=None):
        ...
        self.animation_data_validated = False
        ...
        # ADD THIS LINE HERE - after init_ui() creates the tab_widget
    ...
    def show_correlation_summary(self, data):
        """Show summary of correlation analysis results"""
        
        correlations = []
        variables = [
            ('pressure_df', 'aspect_ratio'),
            ('pressure_df', 'circularity'), 
            ('pressure_df', 'area_pixels'),
            ('velocity_df2', 'aspect_ratio'),
            ('velocity_df2', 'circularity'),
            ('velocity_df2', 'area_pixels')
        ]
        
        for env_var, morph_var in variables:
            if env_var in data.columns and morph_var in data.columns:
                clean_data = data[[env_var, morph_var]].dropna()
                if len(clean_data) > 10:  # Need minimum data points
                    corr = clean_data[env_var].corr(clean_data[morph_var])
                    correlations.append(f"{env_var} vs {morph_var}: r = {corr:.3f}")
        
        summary_text = f"Analysis Summary ({len(data)} cells):\n" + "\n".join(correlations)
        
        QMessageBox.information(self, "Correlation Summary", summary_text)
